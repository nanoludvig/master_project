...
def closest_point_on_lines(x, p, threshold, indices):
	...
	# make a tree for quick nearest neighbor search
	...
	for index in indices:
		...
		# Find all the points within the threshold distance of the selected point x0 excluding the point itself
		indices_close_to_point = tree.query_ball_point(x0, threshold)
		indices_close_to_point = [i for i in indices_close_to_point if i != index]

		for indices_close in indices_close_to_point:
			distance_between_points = np.linalg.norm(x0 - x[indices_close])
			polarity_position_alignment = np.dot(p0,(x[indices_close]-x0)/distance_between_points) 
			polarity_polarity_alignment = np.dot(p0, p[indices_close])
			if (polarity_position_alignment < -0.2) & (polarity_polarity_alignment < -0.2):  # if the polarities are in the opposite direction keep them
				# if the polarities are in the opposite direction keep them
				closest_point = x[indices_close]
				results.append((x0, p0, closest_point, indices_close))
				break
			else:
				pass
...
def thin_line(points, point_cloud_thickness=0.53, iterations=1,sample_points=0):
	...
	# remove points that are too far from each other
	print('new_points.shape',points.shape)
	distances = np.linalg.norm(points[:, None] - points[None, :], axis=-1)
	neighbor_counts = np.sum((distances <= 2) & (distances > 0), axis=1)
...
def connect_triplet_endpoints(sorted_points_collected, radius):
	...
	used_index = []
	if number_of_triplet_comb < 150000:
		valid_triplets = calculate_midpoints_of_three_endpoints(endpoints, points, distance_threshold=radius)
		...
		for triplet, midpoint in valid_triplets:
			...
			used_index.extend([triplet[0], triplet[1], triplet[2]])
...
# Thin & sort points
thinned_points, regression_lines = thin_line(midpoints, point_cloud_thickness=2, iterations=10, sample_points=0)

#sorted_points_collected = sort_points_in_segments(thinned_points, regression_lines, sorted_points_distance=2)

#adjacency_matrix, points = connect_triplet_endpoints(sorted_points_collected, radius=4)
#adjacency_matrix, points = connect_segments(sorted_points_collected, radius=10)
...
#	sorted_marker.set_data(sorted_points, face_color=index_color[idx], size=1, edge_width=0)
